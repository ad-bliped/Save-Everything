...
## SSH 주소로 원격 연결(요청 반영)
받아주신 SSH 주소는 아래가 맞습니다.

`git@github.com:ad-bliped/Save-Everything.git`

### 자동 적용 (CMD)
```bat
cd /d "C:\Users\신예찬\Desktop\APP\Foorink"
scripts\use-ssh-origin.bat
...
> `not a git repository`가 뜨면 현재 위치가 Foorink 폴더가 아닌 상태입니다.
...
2. `'gh'은(는) 내부 또는 외부 명령...이 아닙니다`
   ...
   - 해결 방법:
     ...
     - B안: gh 없이 git으로 받기
       ```bat
       git clone git@github.com:ad-bliped/Save-Everything.git Foorink
       ```
